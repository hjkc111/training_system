<template>
  <div class="training-list-container">
    ...
    <div class="main-content">
        ...
        <el-table :data="trainingList" border stripe>
          ...
          <el-table-column label="完成进度" width="150">
            <template #default="scope">
              <el-progress 
                :percentage="scope.row.project_count > 0 ? Math.round((scope.row.finished_project_count / scope.row.project_count) * 100) : 0"
              ></el-progress>
            </template>
          </el-table-column>
      ...
      <el-empty v-else description="暂无网络布线训练日，点击右上角新建训练日开始训练"></el-empty>

      <!-- 新建训练日弹窗 -->
      <el-dialog v-model="showCreateDialog" title="新建网络布线训练日" width="600px">
...
const getPresetProjects = async () => {
  try {
    ...
    ElMessage.error('获取网络布线项目预设失败')
    // 接口失败时给默认预设项
    presetProjects.value = [
      {
        project_id: 'net_001',
        project_name: '铜缆布线端接',
        project_desc: '完成RJ45水晶头、模块的端接，符合T568A/T568B标准，测试链路通断'
      },
      {
        project_id: 'net_002',
        project_name: '光缆熔接与端接',
        project_desc: '完成光纤切割、熔接，LC/SC/FC连接器端接，测试光衰减值'
      },
      {
        project_id: 'net_003',
        project_name: '综合布线链路测试',
        project_desc: '使用FLUKE测试仪测试铜缆/光缆链路的衰减、串扰、长度等参数'
      },
      {
        project_id: 'net_004',
        project_name: '机柜理线与安装',
        project_desc: '规范完成机柜内配线架、交换机安装，铜缆/光缆理线，标识清晰规范'
      },
      {
        project_id: 'net_005',
        project_name: '综合布线方案设计',
        project_desc: '根据场景需求设计综合布线拓扑，计算点位、链路长度，编写设计文档'
      }
    ]
    selectedProjectIds.value = presetProjects.value.map(item => item.project_id)
  }
...
const getTrainingList = async () => {
  ...
  try {
    ...
    ElMessage.error('获取网络布线项目训练日列表失败')
  }
}
...
const createTrainingDay = async () => {
  ...
  if (selectedProjectIds.value.length === 0) {
    ElMessage.warning('请至少选择一个网络布线训练项目')
    return
  }
  ...
  try {
    ...
    if (res.data.code === 200) {
      ElMessage.success('网络布线项目训练日创建成功！')
      showCreateDialog.value = false
      getTrainingList()
      // 跳转到网络布线项目详情页
      router.push(`/network/training/day/detail/${res.data.training_day_data.training_day_id}`)
    }
  } catch (err) {
    ElMessage.error('创建网络布线项目训练日失败')
  } finally {
