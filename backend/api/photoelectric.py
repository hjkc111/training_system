...
from utils.llm_utils import call_qwen35, call_qwen_project_analysis_network, call_qwen_training_summary,call_qwen_project_analysis_photoelectric
...
async def create_training_day_photoelectric(req: CreateTrainingDayRequest = Body(...)):
    ...
    try:
        req.project_type = "photoelectric"  # 强制标记为光电项目
        ...
        raise HTTPException(status_code=500, detail=f"光电创建训练日失败：{str(e)}")
...
async def get_training_day_list_photoelectric(req: HistoryRequest = Body(...)):
    ...
    try:
        list_data = get_user_training_day_list(req.username, project_type="photoelectric")
        return {
            "code": 200,
            "list": list_data
        }
...
async def get_training_day_detail_photoelectric(req: TrainingDayIdRequest = Body(...)):
    ...
    if training_day.get("project_type") != "photoelectric":
        raise HTTPException(status_code=403, detail="无权限访问非光电项目训练日")
    return {
        "code": 200,
        "detail": training_day
    }
...
async def project_analyze_photoelectric(req: ProjectAnalyzeRequest = Body(...)):
    ...
    analysis_result = call_qwen_project_analysis_photoelectric(
...
async def doc_upload_photoelectric(
    file: UploadFile = File(...),
    chunk: str = Form(None),
    chunks: str = Form(None),
    filename: str = Form(None),
...
async def image_upload_photoelectric(
    file: UploadFile = File(...),
    chunk: str = Form(None),
    chunks: str = Form(None),
    filename: str = Form(None),
...
# ------------------- 光电项目步骤截图上传（复用图片上传逻辑，仅做数量校验） -------------------
@router.post("/step/screenshot/upload")
async def step_screenshot_upload_photoelectric(
    file: UploadFile = File(...),
    chunk: str = Form(None),
    chunks: str = Form(None),
    filename: str = Form(None),
    ...
    return await image_upload_photoelectric(
        file=file,
        chunk=chunk,
        chunks=chunks,
        filename=filename,
...
async def project_analyze_doc_photoelectric(req: ProjectAnalyzeRequest = Body(...)):
    # 基础校验（和视频分析一致）
    training_day = get_training_day(req.training_day_id, req.username)
    if not training_day:
        raise HTTPException(status_code=404, detail="训练日不存在或无权限")
...
async def project_analyze_image_photoelectric(
...
async def project_analyze_step_photoelectric(
    training_day_id: str = Body(...),
    project_id: str = Body(...),
    filenames: list = Body(...),
    username: str = Body(...)
