...
import shutil
import cv2
from config import ALLOWED_VIDEO_TYPES, MAX_VIDEO_SIZE, UPLOAD_DIR, MEDIA_EXTRACT_DIR, PRESET_PROJECTS
from models import (
...
async def upload_video_network(
    ...
    # 复制原main.py中 /api/video/upload 的所有代码（完全不变）
...
async def analyze_video_network(analysis_req: AnalysisRequest = Body(...)):
    # 复制原main.py中 /api/video/analyze 的所有代码（完全不变）
...
async def create_training_day_network(req: CreateTrainingDayRequest = Body(...)):
    # 复制原main.py中 /api/training/day/create 的所有代码
    try:
...
async def get_training_day_list_network(req: HistoryRequest = Body(...)):
    # 复制原main.py中 /api/training/day/list 的所有代码
    try:
...
async def get_training_day_detail_network(req: TrainingDayIdRequest = Body(...)):
    # 复制原main.py中 /api/training/day/detail 的所有代码
...
async def project_analyze_network(req: ProjectAnalyzeRequest = Body(...)):
    # 复制原main.py中 /api/training/project/analyze 的所有代码
    training_day = get_training_day(req.training_day_id, req.username)
    if not training_day:
        raise HTTPException(status_code=404, detail="训练日不存在或无权限")
    target_project = None
...
async def training_day_summary_network(req: TrainingDaySummaryRequest = Body(...)):
    # 复制原main.py中 /api/training/day/summary 的所有代码
    training_day = get_training_day(req.training_day_id, req.username)
    if not training_day:
        raise HTTPException(status_code=404, detail="训练日不存在或无权限")
...
async def get_preset_projects_network():
    # 复制原main.py中 /api/training/project/preset 的所有代码
    return {
        "code": 200,
...
# ------------------- 网络布线历史分析记录 -------------------
@router.post("/video/analysis/history")
async def get_analysis_history_network(req: HistoryRequest = Body(...)):
    # 复制原main.py中 /api/video/analysis/history 的所有代码
    username = req.username
    records = get_analysis_history(username)
    return {
        "code": 200,
